import { z } from 'zod';
import { resolverConfiguracionNegocio } from '../resolverConfiguracionNegocio';
...
describe('resolverConfiguracionNegocio', () => {
  test('fusiona solo objetos planos y claves conocidas e ignora undefined', () => {
    ...
    expect(resolverConfiguracionNegocio(rawConfig, defaultConfig)).toEqual({
      ticket: {
        header: 'Panadería Aurora',
        footer: 'Gracias',
        includeQr: false,
    ...
    expect(resolverConfiguracionNegocio(instanciaConPrototipo, defaultConfig)).toEqual(
      defaultConfig
    );
    expect(resolverConfiguracionNegocio(null, defaultConfig)).toEqual(defaultConfig);
    expect(resolverConfiguracionNegocio(['config'], defaultConfig)).toEqual(defaultConfig);
  });

  test('acepta null solo cuando la configuración base permite null e ignora tipos incorrectos', () => {
    const result = resolverConfiguracionNegocio(
      {
        ticket: {
          header: null,
          footer: 10,
    ...
    const conMetadata = resolverConfiguracionNegocio(
      {
        ticket: {
  ...
  test('reemplaza arreglos completos cuando el default permite inferir un contrato homogéneo', () => {
    ...
    const result = resolverConfiguracionNegocio(rawConfig, defaultConfig);
  ...
  test('deja que el schema valide arreglos sin contrato inferible y valores no-null sobre null', () => {
    ...
    const result = resolverConfiguracionNegocio(rawConfig, defaultConfig, schemaConfig);
  ...
  test('no muta entradas y devuelve objetos y arreglos nuevos a toda profundidad', () => {
    ...
    const result = resolverConfiguracionNegocio(raw, base);
