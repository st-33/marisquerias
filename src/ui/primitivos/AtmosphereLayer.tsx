...
export function AtmosphereLayer({ variant = 'command', style }: AtmosphereLayerProps) {
  ...
  return (
    <View pointerEvents="none" style={[styles.layer, style]}>
      <LinearGradient colors={palette.main} end={{ x: 1, y: 1 }} start={{ x: 0, y: 0 }} style={[styles.orb, styles.mainOrb]} />
      <LinearGradient colors={palette.accent} end={{ x: 1, y: 1 }} start={{ x: 0, y: 0 }} style={[styles.orb, styles.accentOrb]} />
      <LinearGradient colors={palette.ember} end={{ x: 0, y: 1 }} start={{ x: 1, y: 0 }} style={[styles.orb, styles.emberOrb]} />
...
const styles = StyleSheet.create({
  ...
  topHairline: { backgroundColor: 'rgba(255,255,255,0.055)', height: 1, left: 0, position: 'absolute', right: 0, top: 0 },
});
