...
import { logger } from '../../../plataforma/core/monitoring';
import { useStore } from '../../../plataforma/core/store';
import { normalizePathname } from '../../../plataforma/core/navigation/normalizePathname';
...
export function usePuenteAccionesFlotantes(config: FabConfig) {
  ...
  const configSignature = [
    ...
    config.visibleCount ?? '',
    config.position ?? '',
    config.items.map((item) => `${item.key}:${item.label}:${item.color ?? ''}`).join('|'),
  ].join('::');
