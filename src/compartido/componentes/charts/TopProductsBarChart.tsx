    ...
    <View style={styles.container}>
      ...
      <View style={styles.chartContainer}>
        {ranking.map((product, index) => {
          ...
          return (
            <Animated.View entering={FadeInRight.delay(index * 58).duration(260)} key={product.id} style={styles.row}>
              <View style={[styles.rank, { backgroundColor: `${palette[0]}20`, borderColor: `${palette[0]}66` }]}>
                <Text style={[styles.rankText, { color: palette[0] }]}>{String(index + 1).padStart(2, '0')}</Text>
              </View>
              <View style={styles.productColumn}>
                <View style={styles.nameRow}><Text numberOfLines={1} style={styles.productName}>{product.nombre}</Text>{index === 0 && <Text style={styles.leaderTag}>LÍDER</Text>}</View>
                <View style={styles.barTrack}>
                  <LinearGradient colors={palette} end={{ x: 1, y: 0 }} start={{ x: 0, y: 0 }} style={[styles.bar, { width: `${share}%` }]} />
                </View>
              </View>
              <View style={[styles.scoreBox, { borderColor: `${palette[0]}38`, backgroundColor: isElite ? 'rgba(7,11,18,0.48)' : 'rgba(255,255,255,0.04)' }]}>
...
const styles = StyleSheet.create({
  ...
  rank: { alignItems: 'center', borderRadius: 9, borderWidth: 1, height: 32, justifyContent: 'center', width: 32 },
  ...
  leaderTag: { backgroundColor: 'rgba(244,201,93,0.14)', borderRadius: 5, color: '#F4C95D', fontSize: 7, fontWeight: '900', letterSpacing: 0.7, paddingHorizontal: 5, paddingVertical: 2 },
  barTrack: { backgroundColor: 'rgba(166,182,208,0.11)', borderRadius: 5, height: 7, overflow: 'hidden' },
  bar: { borderRadius: 5, height: '100%' },
  scoreBox: { alignItems: 'flex-end', borderRadius: 9, borderWidth: 1, minWidth: 42, paddingHorizontal: 6, paddingVertical: 5 },
  salesValue: { color: '#F6F2E8', fontSize: 12, fontWeight: '900' },
  salesLabel: { color: '#7D899A', fontSize: 6, fontWeight: '900', letterSpacing: 0.65, marginTop: 1 },
